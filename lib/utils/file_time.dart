...
DynamicLibrary get _kernel32Lib {
...
final _CreateFileWDart _createFileW =
    _kernel32Lib.lookupFunction<_CreateFileWNative, _CreateFileWDart>(
        'CreateFileW');
...
final _GetFileTimeDart _getFileTime =
    _kernel32Lib.lookupFunction<_GetFileTimeNative, _GetFileTimeDart>(
        'GetFileTime');
...
final _CloseHandleDart _closeHandle =
    _kernel32Lib.lookupFunction<_CloseHandleNative, _CloseHandleDart>(
        'CloseHandle');

/// 获取目录创建时间 (Windows 专用)。
/// dart:io 的 FileStat 无创建时间, 用 CreateFileW + GetFileTime 读取。
/// 其他平台 (Android 等) 无此 API, 返回 null (调用方回退 mtime)。
/// 失败返回 null。
