...
- `config/`, `data/`, `history/` are gitignored runtime state. `config/config.json` holds `voiceWorkRoot`, `closeToTray`, `windowEffect` (`transparent`/`acrylic`/`opaque`), `themeColorMode` (`custom`/`cover`), `themeSeedColor` (`#RRGGBB`), `accentColorMode` (`follow`/`custom`), `accentColor` (`#RRGGBB`); legacy `liquidGlass`/`followCoverTheme` keys are auto-migrated on read (`resolveWindowEffect`/`resolveThemeColorMode` in `lib/services/ui/theme/theme_provider.dart`) and removed on write; `history/last_played.json` is play-position memory.
