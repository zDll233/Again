...
- DB refresh (`lib/services/database/voice_updater.dart`): `scanRoot` runs via `compute` in a background isolate; unchanged works are skipped by comparing a `scanSignature` (child count + max child mtime — Windows dir mtimes don't update on child add/remove, so a bare mtime is unreliable); the DB sync runs in one transaction. FK constraints are ON — deletions must respect order tVoiceCV → tVoiceItem → TVoiceWork → category. DB schema is at version 2 (`scanSignature` column, added by migration; run `dart run build_runner build` after editing `database.dart`).
